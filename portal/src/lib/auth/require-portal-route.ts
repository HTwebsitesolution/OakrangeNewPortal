...
export type PortalRouteSession = {
  supabase: SupabaseClient;
  user: { id: string };
  profile: SessionProfile;
};

export async function requirePortalRoute(): Promise<{
  session: PortalRouteSession | null;
  ...
  if (profile.role !== "site_manager" && profile.role !== "customer_user") {
    return {
      session: null,
      response: NextResponse.json({ error: "Portal access required." }, { status: 403 }),
    };
  }
